...
- 不加新业务功能
- 不动业务功能 UI 文案
- 不动 prisma schema (除非审计/可观测性需要的最小字段; 涉及 schema 改动需 ADR 单独评审)
- 不重写前端框架
- 不换数据库/队列
- 不做压测专项 sprint (随 S3/S4 自然进行)
- **澄清**: MinIO 落地属于"补全 V1 既有 stub (oss.service.ts 60 行)"而非新增业务功能
- **澄清**: 加密升级 AES-256-CBC → AES-256-GCM 属于代码层改造, 不改 prisma schema
