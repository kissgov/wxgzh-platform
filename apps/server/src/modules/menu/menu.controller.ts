...
import { SaveMenuDto, CreateMenuTemplateDto } from './menu.dto';
...
export class MenuController {
  ...
  async save(
    ...
    @Body() dto: SaveMenuDto,
  ) {
    const data = await this.menuService.saveDraft(tenantId, authorizerId, dto);
  ...
  async createTemplate(@TenantId() tenantId: string, @Body() dto: CreateMenuTemplateDto) {
    const data = await this.menuService.createTemplate(tenantId, dto);
